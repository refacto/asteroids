...
static Vector2 perpendicularNormVector(Vector2 original) {
...
struct Asteroid *asteroid_split(struct Asteroid *asteroid,
	...
	Vector2 newVelocity = perpendicularNormVector(damageDirection);
	float newSpeed = specs[newSize].speed;
	left->object.velocity = Vector2Scale(newVelocity, newSpeed);
	right->object.velocity = Vector2Scale(newVelocity, -newSpeed);
