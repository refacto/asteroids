...
void title_init(struct Title *title, struct FontLoader *fontLoader) 
{
	*title = (struct Title)
	...
	Vector2 screen_dimensions = screenDimensions_get();
	asteroidShower_init(&title->asteroidShower, screen_dimensions);
}
